    ...
    if (!("SVGPathSeg" in window)) {
        ...
        if (!("getPathSegAtLength" in window.SVGPathElement.prototype)) {
            ...
            window.SVGPathElement.prototype.getPathSegAtLength = function (distance) {
                ...
                if (lastPathSegment <= 0)="" return="" 0;="" do="" {="" measurementelement.pathseglist.removeitem(lastpathsegment);="" if="" (distance=""> measurementElement.getTotalLength())
                        break;
                    lastPathSegment--;
            ...
            Source.prototype._isCurrentSpace = function () {
                ...
                if (this._currentIndex < this._endIndex && this._string.charAt(this._currentIndex) == ".") {
                    ...
                    if (this._currentIndex >= this._endIndex || this._string.charAt(this._currentIndex) < "0" || this._string.charAt(this._currentIndex) > "9")
                        return undefined;
                    while (this._currentIndex < this._endIndex && this._string.charAt(this._currentIndex) >= "0" && this._string.charAt(this._currentIndex) <= 1="" "9")="" {="" frac="" *="10;" decimal="" +="(this._string.charAt(this._currentIndex)" -="" "0")="" frac;="" this._currentindex="" }="" read="" the="" exponent="" part.="" if="" (this._currentindex="" !="startIndex" &&="" <="" this._endindex="" (this._string.charat(this._currentindex)="=" "e"="" ||="" this._string.charat(this._currentindex)="=" "e")="" (this._string.charat(this._currentindex="" 1)="" this._string.charat(this._currentindex="" ))="" this._currentindex++;="" sign="" of="" exponent.="" "+")="" else="" "-")="" expsign="-1;" there="" must="" be="" an="">= this._endIndex || this._string.charAt(this._currentIndex) < "0" || this._string.charAt(this._currentIndex) > "9")
                        return undefined;

                    while (this._currentIndex < this._endIndex && this._string.charAt(this._currentIndex) >= "0" && this._string.charAt(this._currentIndex) <= "9")="" {="" exponent="" *="10;" +="(this._string.charAt(this._currentIndex)" -="" "0");="" this._currentindex++;="" }="" var="" number="integer" decimal;="" if="" (exponent)="" expsign="" exponent);="" (startindex="=" this._currentindex)="" return="" undefined;="" this._skipoptionalspacesordelimiter();="" number;="" };="" source.prototype._parsearcflag="function" ()="" (this._currentindex="">= this._endIndex)
                    return undefined;
                var flag = false;
...
ChartInternal.prototype.updateRadius = function () {
    ...
    $$.gaugeArcWidth = w ? w : (gaugeArcWidth <= 0="" 2="" $$.radius="" -="" $$.innerradius="" ?="" :="" (gaugearcwidth="" <="$$.radius" gaugearcwidth="" $$.radius));="" };="" chartinternal.prototype.getpadangle="function()" {="" if="" (this.hastype('pie'))="" return="" this.config.pie_padangle="" ||="" 0;="" }="" else="" (this.hastype('donut'))="" this.config.donut_padangle="" chartinternal.prototype.updatearc="function" ()="" var="" $$="this;" $$.svgarc="$$.getSvgArc();" $$.svgarcexpanded="$$.getSvgArcExpanded();" $$.svgarcexpandedsub="$$.getSvgArcExpanded(0.98);" chartinternal.prototype.updateangle="function" (d)="" config="$$.config," found="false," index="0," gmin,="" gmax,="" gtic,="" gvalue;="" (!config)="" null;="" $$.pie($$.filtertargetstoshow($$.data.targets)).foreach(function="" (t)="" (!="" &&="" t.data.id="==" d.data.id)="" d="t;" d.index="index;" index++;="" });="" (isnan(d.startangle))="" d.startangle="0;" (isnan(d.endangle))="" d.endangle="d.startAngle;" ($$.isgaugetype(d.data))="" gmin="config.gauge_min;" gmax="config.gauge_max;" gtic="(Math.PI" *="" (config.gauge_fullcircle="" 1))="" (gmax="" gmin);="" gvalue="d.value" d.value="" +="" chartinternal.prototype.getsvgarc="function" hasgaugetype="$$.hasType('gauge')," singlearcwidth="$$.gaugeArcWidth" $$.filtertargetstoshow($$.data.targets).length,="" arc="$$.d3.arc().outerRadius(function(d)" $$.radius;="" }).innerradius(function(d)="" (d.index="" 1)="" $$.innerradius;="" }),="" newarc="function" (d,="" withoutupdate)="" updated;="" (withoutupdate)="" arc(d);="" for="" interpolate="" updated="$$.updateAngle(d);" arc(updated)="" "m="" 0";="" todo:="" extends="" all="" function="" newarc.centroid="arc.centroid;" newarc;="" chartinternal.prototype.getsvgarcexpanded="function" (rate)="" rate="rate" 1;="" expandwidth="Math.min($$.radiusExpanded" $$.radius,="" 0.8="" (1="" rate)="" 100),="" $$.radiusexpanded="" rate;="" }).innerradius(function(d){="" chartinternal.prototype.getarc="function" withoutupdate,="" force)="" force="" this.isarctype(d.data)="" this.svgarc(d,="" chartinternal.prototype.transformforarclabel="function" c,="" x,="" y,="" h,="" ratio,="" translate="" ,="" hasgauge="$$.hasType('gauge');" (updated="" !hasgauge)="" c="this.svgArc.centroid(updated);" x="isNaN(c[0])" c[0];="" y="isNaN(c[1])" c[1];="" h="Math.sqrt(x" y);="" ($$.hastype('donut')="" config.donut_label_ratio)="" ratio="isFunction(config.donut_label_ratio)" config.donut_label_ratio(d,="" h)="" config.donut_label_ratio;="" ($$.hastype('pie')="" config.pie_label_ratio)="" config.pie_label_ratio(d,="" config.pie_label_ratio;="" (36=""> 0.375 ? 1.175 - 36 / $$.radius : 0.8) * $$.radius / h : 0;
        }
    ...
    return targets.map(function (t) {
        return {
            ...
            values: t.values.filter(function (v) {
                return xDomain[0] <= v.x="" &&="" <="xDomain[1];" })="" };="" });="" chartinternal.prototype.hasdatalabel="function" ()="" {="" var="" config="this.config;" if="" (typeof="" config.data_labels="==" 'boolean'="" config.data_labels)="" return="" true;="" }="" else="" 'object'="" notempty(config.data_labels))="" false;="" chartinternal.prototype.getdatalabellength="function" (min,="" max,="" key)="" $$="this," lengths="[0," 0],="" paddingcoef="1.3;" $$.selectchart.select('svg').selectall('.dummy')="" .data([min,="" max])="" .enter().append('text')="" .text(function="" (d)="" $$.datalabelformat(d.id)(d);="" .each(function="" (d,="" i)="" lengths[i]="getBBox(this)[key]" *="" paddingcoef;="" .remove();="" lengths;="" **="" returns="" true="" the="" given="" data="" point="" is="" not="" arc="" type,="" otherwise="" false.="" @param="" {object}="" d="" @return="" {boolean}="" chartinternal.prototype.isnonearc="function" this.hastarget(this.data.targets,="" d.id);="" chartinternal.prototype.isarc="function" 'data'="" in="" d.data.id);="" find="" closest="" from="" pos="" among="" targets="" or="" undefined="" none="" satisfies="" conditions.="" {array}="" an="" [x,y]="" coordinate="" {object|undefined}="" chartinternal.prototype.findclosestfromtargets="function" (targets,="" pos)="" const="" for="" each="" target,="" candidates="targets" .map((t)=""> $$.findClosest(t.values, pos, $$.config.tooltip_horizontal ? $$.horizontalDistance.bind($$) : $$.dist.bind($$), $$.config.point_sensitivity))
        .filter((v) => v);
...
ChartInternal.prototype.trimXDomain = function (domain) {
    ...
    if (domain[0] <= 0="" 2="" 4="" min)="" {="" domain[1]="+domain[1]" +="" (min="" -="" domain[0]);="" domain[0]="min;" }="" if="" (max="" <="domain[1])" (domain[1]="" max);="" return="" domain;="" };="" chartinternal.prototype.drag="function" (mouse)="" var="" $$="this," config="$$.config," main="$$.main," d3="$$.d3;" sx,="" sy,="" mx,="" my,="" minx,="" maxx,="" miny,="" maxy;="" ($$.hasarctype())="" return;="" (!config.data_selection_enabled)="" do="" nothing="" not="" selectable="" (!config.data_selection_multiple)="" skip="" when="" single="" selection="" because="" drag="" is="" used="" for="" multiple sx="$$.dragStart[0];" sy="$$.dragStart[1];" mx="mouse[0];" my="mouse[1];" minx="Math.min(sx," mx);="" maxx="Math.max(sx," miny="(config.data_selection_grouped)" ?="" $$.margin.top="" :="" math.min(sy,="" my);="" maxy="(config.data_selection_grouped)" $$.height="" math.max(sy,="" main.select('.'="" class.dragarea)="" .attr('x',="" minx)="" .attr('y',="" miny)="" .attr('width',="" .attr('height',="" miny);="" todo:="" binary="" search="" xs="" main.selectall('.'="" class.shapes).selectall('.'="" class.shape)="" .filter(function="" (d)="" config.data_selection_isselectable(d);="" })="" .each(function="" (d,="" i)="" shape="d3.select(this)," isselected="shape.classed(CLASS.SELECTED)," isincluded="shape.classed(CLASS.INCLUDED)," _x,="" _y,="" _w,="" _h,="" toggle,="" iswithin="false," box;="" (shape.classed(class.circle))="" _x="shape.attr("cx")" *="" 1;="" _y="shape.attr("cy")" toggle="$$.togglePoint;" &&="" else="" (shape.classed(class.bar))="" box="getPathBox(this);" _w="box.width;" _h="box.height;" ||="" !(maxy="" line="" area="" supported="" yet="" (iswithin="" ^="" isincluded)="" shape.classed(class.included,="" !isincluded);="" included="" unincluded="" callback="" here="" shape.classed(class.selected,="" !isselected);="" toggle.call($$,="" !isselected,="" shape,="" d,="" i);="" });="" chartinternal.prototype.dragstart="function" (!="" config.data_selection_enabled)="" $$.dragstart="mouse;" $$.main.select('.'="" class.chart).append('rect')="" .attr('class',="" .style('opacity',="" 0.1);="" $$.dragging="true;" chartinternal.prototype.dragend="function" ()="" .transition().duration(100)="" 0)="" .remove();="" $$.main.selectall('.'="" .classed(class.included,="" false);="" chartinternal.prototype.getyformat="function" (forarc)="" formatfory="forArc" !$$.hastype('gauge')="" $$.defaultarcvalueformat="" $$.yformat,="" formatfory2="forArc" $$.y2format;="" function="" (v,="" ratio,="" id)="" format="$$.axis.getId(id)" =="=" 'y2'="" formatfory;="" format.call($$,="" v,="" ratio);="" chartinternal.prototype.yformat="function" (v)="" config.axis_y_tick_format="" $$.defaultvalueformat;="" format(v);="" chartinternal.prototype.y2format="function" config.axis_y2_tick_format="" chartinternal.prototype.defaultvalueformat="function" isvalue(v)="" +v="" "";="" chartinternal.prototype.defaultarcvalueformat="function" ratio)="" (ratio="" 100).tofixed(1)="" '%';="" chartinternal.prototype.datalabelformat="function" (targetid)="" data_labels="$$.config.data_labels," format,="" defaultformat="function" find="" according="" to="" axis="" id="" (typeof="" data_labels.format="==" 'function')="" 'object')="" (data_labels.format[targetid])="" true="" data_labels.format[targetid];="" '';="" format;="" chartinternal.prototype.initgrid="function" $$.grid="$$.main.append('g')" .attr("clip-path",="" $$.clippathforgrid)="" class.grid);="" (config.grid_x_show)="" $$.grid.append("g").attr("class",="" class.xgrids);="" (config.grid_y_show)="" $$.grid.append('g').attr('class',="" class.ygrids);="" (config.grid_focus_show)="" $$.grid.append('g')="" .attr("class",="" class.xgridfocus)="" .append('line')="" class.xgridfocus);="" $$.xgrid="d3.selectAll([]);" (!config.grid_lines_front)="" $$.initgridlines();="" chartinternal.prototype.initgridlines="function" $$.gridlines="$$.main.append('g')" class.grid="" '="" class.gridlines);="" $$.gridlines.append('g').attr("class",="" class.xgridlines);="" $$.gridlines.append('g').attr('class',="" class.ygridlines);="" $$.xgridlines="d3.selectAll([]);" chartinternal.prototype.updatexgrid="function" (withoutupdate)="" xgriddata="$$.generateGridData(config.grid_x_type," $$.x),="" tickoffset="$$.isCategorized()" $$.xaxis.tickoffset()="" 0;="" $$.xgridattr="config.axis_rotated" 'x1':="" 0,="" 'x2':="" $$.width,="" 'y1':="" $$.x(d)="" tickoffset;="" },="" 'y2':="" $$.xgridattr.opacity="function" pos="+d3.select(this).attr(config.axis_rotated" 'y1'="" 'x1');="" (config.axis_rotated="" xgrid="$$.main.select('.'" class.xgrids).selectall('.'="" class.xgrid)="" .data(xgriddata);="" xgridenter="xgrid.enter().append('line')" .attr('x1',="" $$.xgridattr.x1)="" .attr('x2',="" $$.xgridattr.x2)="" .attr('y1',="" $$.xgridattr.y1)="" .attr('y2',="" $$.xgridattr.y2)="" .style("opacity",="" 0);="" (!withoutupdate)="" $$.xgridattr.opacity);="" xgrid.exit().remove();="" chartinternal.prototype.updateygrid="function" gridvalues="$$.yAxis.tickValues()" $$.y.ticks(config.grid_y_ticks);="" ygrid="$$.main.select('.'" class.ygrids).selectall('.'="" class.ygrid)="" .data(gridvalues);="" ygridenter="ygrid.enter().append('line')" x1,="" x2,="" y1,="" y2,="" opacity="" need="" be="" set="" maybe="" class.ygrid);="" $$.ygrid="ygridEnter.merge(ygrid);" .attr("x1",="" config.axis_rotated="" $$.y="" .attr("x2",="" $$.width)="" .attr("y1",="" $$.y)="" .attr("y2",="" $$.y);="" ygrid.exit().remove();="" $$.smoothlines($$.ygrid,="" 'grid');="" chartinternal.prototype.gridtextanchor="function" d.position="" "end";="" chartinternal.prototype.gridtextdx="function" 'start'="" 'middle'="" -4;="" chartinternal.prototype.xgridtextx="function" -this.height="" chartinternal.prototype.ygridtextx="function" this.width="" this.width;="" chartinternal.prototype.updategrid="function" (duration)="" xgridline,="" xgridlineenter,="" ygridline,="" ygridlineenter,="" xv="$$.xv.bind($$)," yv="$$.yv.bind($$)," xgridtextx="$$.xGridTextX.bind($$)," ygridtextx="$$.yGridTextX.bind($$);" hide="" arc="" type="" $$.grid.style('visibility',="" $$.hasarctype()="" 'hidden'="" 'visible');="" main.select('line.'="" class.xgridfocus).style("visibility",="" "hidden");="" $$.updatexgrid();="" xgridline="main.select('.'" class.xgridlines).selectall('.'="" class.xgridline)="" .data(config.grid_x_lines);="" enter="" xgridlineenter="xgridLine.enter().append('g')" class.xgridline="" (d['class']="" d['class']="" '');="" xgridlineenter.append('line')="" xv)="" $$.width="" $$.height)="" xgridlineenter.append('text')="" .attr("text-anchor",="" $$.gridtextanchor)="" .attr("transform",="" ""="" "rotate(-90)")="" .attr("x",="" xgridtextx)="" .attr("y",="" .attr('dx',="" $$.gridtextdx)="" .attr('dy',="" -5)="" udpate="" done="" in="" d3.transition()="" of="" the="" end="" this="" exit="" xgridline.exit().transition().duration(duration)="" y-grid="" $$.updateygrid();="" ygridline="main.select('.'" class.ygridlines).selectall('.'="" class.ygridline)="" .data(config.grid_y_lines);="" ygridlineenter="ygridLine.enter().append('g')" class.ygridline="" ygridlineenter.append('line')="" yv)="" ygridlineenter.append('text')="" "rotate(-90)"="" "")="" ygridtextx)="" update="" $$.ygridlines="ygridLineEnter.merge(ygridLine);" $$.ygridlines.select('line')="" .transition().duration(duration)="" 1);="" $$.ygridlines.select('text')="" $$.xgridtextx.bind($$)="" $$.ygridtextx.bind($$))="" .text(function="" d.text;="" ygridline.exit().transition().duration(duration)="" chartinternal.prototype.redrawgrid="function" (withtransition,="" transition)="" lines="$$.xgridLines.select('line')," texts="$$.xgridLines.select('text');" [="" (withtransition="" lines.transition(transition)="" lines)="" 1),="" texts.transition(transition)="" texts)="" $$.ygridtextx.bind($$)="" $$.xgridtextx.bind($$))="" 1)="" ];="" chartinternal.prototype.showxgridfocus="function" (selecteddata)="" datatoshow="selectedData.filter(function" d="" isvalue(d.value);="" }),="" focusel="$$.main.selectAll('line.'" class.xgridfocus),="" xx="$$.xx.bind($$);" config.tooltip_show)="" stanford="" plot="" exists="" ($$.hastype('stanford')="" $$.hasarctype())="" .style("visibility",="" "visible")="" .data([datatoshow[0]])="" .attr(config.axis_rotated="" 'x1',="" xx)="" 'x2',="" xx);="" $$.smoothlines(focusel,="" chartinternal.prototype.hidexgridfocus="function" this.main.select('line.'="" chartinternal.prototype.updatexgridfocus="function" $$.main.select('line.'="" -10)="" -10="" $$.height);="" chartinternal.prototype.generategriddata="function" (type,="" scale)="" griddata="[]," xdomain,="" firstyear,="" lastyear,="" i,="" ticknum="$$.main.select("."" class.axisx).selectall('.tick').size();="" (type="==" 'year')="" xdomain="$$.getXDomain();" firstyear="xDomain[0].getFullYear();" lastyear="xDomain[1].getFullYear();" (i="firstYear;" i="" i++)="" griddata.push(new="" date(i="" '-01-01="" 00:00:00'));="" (griddata.length=""> tickNum) { // use only int
...
ChartInternal.prototype.lineWithRegions = function (d, x, y, _regions) {
    ...
    function isWithinRegions(x, regions) {
        ...
        for (i = 0; i < regions.length; i++) {
            if (regions[i].start < x && x <= regions[i].end)="" {="" return="" true;="" }="" false;="" check="" start="" end="" of="" regions="" if="" (isdefined(_regions))="" for="" (i="0;" i="" <="" _regions.length;="" i++)="" regions[i]="{};" (isundefined(_regions[i].start))="" regions[i].start="d[0].x;" else="" ?="" $$.parsedate(_regions[i].start)="" :="" _regions[i].start;="" (isundefined(_regions[i].end))="" regions[i].end="d[d.length" -="" 1].x;="" $$.parsedate(_regions[i].end)="" _regions[i].end;="" set="" scales="" xvalue="config.axis_rotated" function="" (d)="" y(d.value);="" x(d.x);="" };="" yvalue="config.axis_rotated" define="" svg="" generator="" region="" generatem(points)="" 'm'="" +="" points[0][0]="" '="" points[0][1]="" points[1][0]="" points[1][1];="" ($$.istimeseries())="" swithregion="function" (d0,="" d1,="" j,="" diff)="" var="" x0="d0.x.getTime()," x_diff="d1.x" d0.x,="" xv0="new" date(x0="" *="" j),="" xv1="new" (j="" diff)),="" points;="" (config.axis_rotated)="" points="[[y(yp(j))," x(xv0)],="" [y(yp(j="" x(xv1)]];="" y(yp(j))],="" [x(xv1),="" y(yp(j="" diff))]];="" generatem(points);="" true),="" x(xp(j))],="" diff),="" x(xp(j="" [x(xp(j="" generate="" d.length;="" draw="" as="" normal="" (isundefined(regions)="" ||="" !="" iswithinregions(d[i].x,="" regions))="" s="" xvalue(d[i])="" "="" yvalue(d[i]);="" with="" todo:="" fix="" horizotal="" charts="" xp="$$.getScale(d[i" 1].x="" xoffset,="" d[i].x="" $$.istimeseries());="" yp="$$.getScale(d[i" 1].value,="" d[i].value);="" dx="x(d[i].x)" x(d[i="" 1].x);="" dy="y(d[i].value)" y(d[i="" 1].value);="" dd="Math.sqrt(Math.pow(dx," 2)="" math.pow(dy,="" 2));="" diff="2" dd;="" diffx2="diff" 2;="" j="" 1],="" d[i],="" diff);="" prev="d[i].x;" s;="" chartinternal.prototype.updatearea="function" (durationforexit)="" $$="this," d3="$$.d3;" mainarea="$$.main.selectAll('.'" class.areas).selectall('.'="" class.area)="" .data($$.linedata.bind($$));="" mainareaenter="mainArea.enter().append('path')" .attr("class",="" $$.classarea.bind($$))="" .style("fill",="" $$.color)="" .style("opacity",="" ()="" $$.orgareaopacity="+d3.select(this).style('opacity');" 0;="" });="" $$.mainarea="mainAreaEnter.merge(mainArea)" $$.orgareaopacity);="" mainarea.exit().transition().duration(durationforexit)="" .style('opacity',="" 0);="" chartinternal.prototype.redrawarea="function" (drawarea,="" withtransition,="" transition)="" [="" (withtransition="" this.mainarea.transition(transition)="" this.mainarea)="" .attr("d",="" drawarea)="" this.color)="" this.orgareaopacity)="" ];="" chartinternal.prototype.generatedrawarea="function" (areaindices,="" issub)="" config="$$.config," area="$$.d3.area()," getpoints="$$.generateGetAreaPoints(areaIndices," issub),="" yscalegetter="isSub" $$.getsubyscale="" $$.getyscale,="" (issub="" $$.subxx="" $$.xx).call($$,="" d);="" },="" value0="function" (d,="" i)="" config.data_groups.length=""> 0 ? getPoints(d, i)[0][1] : yScaleGetter.call($$, d.id)($$.getAreaBaseValue(d.id));
        },
        value1 = function (d, i) {
...
var getRegionArea = function(points) { // thanks to: https://stackoverflow.com/questions/16282330/find-centerpoint-of-polygon-in-javascript
    var area = 0,
        i,
        j,
        point1,
...
var pointInRegion = function(point, region) { // thanks to: http://bl.ocks.org/bycoffe/5575904
